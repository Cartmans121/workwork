...
if ($xsl_dir_id)
{
	...
	if (!is_null($oXslDir->id))
	{
		...
		do
		{
			...
			$aBreadcrumbs[] = Core::factory('Admin_Form_Entity_Breadcrumb')
				->name($oXslDir->name)
				->href(
...
{
	$oXsl_Controller_Edit = new Xsl_Controller_Edit(
		$oAdmin_Form_Action
	);
...
{
	$oXslDirControllerApply = new Admin_Form_Action_Controller_Type_Apply
	(
